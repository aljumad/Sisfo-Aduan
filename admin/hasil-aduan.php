...
?>
	...
	<h3 class="text-center">DAFTAR HASIL PENGADUAN</h3>
	<hr>
	<h6 class="text-center">Hasil Pengaduan Per Periode</h6>
...
<?php 
    ...
    if(!isset($_POST['filter']) or isset($_POST['semua'])) {
    	...
    	$data = mysqli_query($koneksi,"SELECT * FROM pengaduan WHERE status='Selesai' ORDER BY id_aduan DESC");
	}	
	else {
		if (($_POST['dari'] == '') and ($_POST['sampai']  == '')) {
			$data = mysqli_query($koneksi,"SELECT * FROM pengaduan WHERE status='Selesai' ORDER BY id_aduan DESC");
		...
		$dari = $_POST['dari'];
		$sampai = $_POST['sampai'];
		$data = mysqli_query($koneksi,"SELECT * FROM pengaduan WHERE status='Selesai' and tgl_aduan between 
...
?>
           	<tr class="text-center">
	            ...
	            <td>
	            	<a href="cetaklampp.php?id=<?php echo $id ?>" target="_blank" class="btn btn-info">LAMPIRAN</a>
	            </td>
	        </tr>
<?php
    }
	...
	<div class="row text-center w-auto mx-auto  justify-content-center">
		<?php 
			if (isset($_POST['filter'])) {
				$tgl1 = $_POST['dari'];
				$tgl2 = $_POST['sampai'];
				echo '<a href="cetakhasil.php?dari='.$tgl1.'&sampai='.$tgl2.'" target="_blank" class="btn btn-danger">CETAK</a>';
			}
			else {
				echo '<a href="cetakhasil.php" target="_blank" class="btn btn-danger">CETAK</a>';
			}
		 ?>

<!-- <a href="cetakhasil.php" target="_blank" class="btn btn-danger">CETAK</a> -->
	</div>
